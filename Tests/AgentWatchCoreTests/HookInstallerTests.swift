...
final class HookInstallerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        setenv("AGENTWATCH_USER_HOME", home.path, 1)
        setenv("AGENTWATCH_HOME", home.appendingPathComponent(".agentwatch").path, 1)
    }

    override func tearDown() {
        unsetenv("AGENTWATCH_USER_HOME")
        unsetenv("AGENTWATCH_HOME")
    ...
    func testClaudeInstallMergesIsIdempotentAndUninstallRestores() throws {
        ...
        let plan = try HookInstaller.plan(.claude, install: true, reporter: "/r/agentwatch-report")
        ...
        XCTAssertEqual(ours?["command"] as? String, "'/r/agentwatch-report' --agent claude")
        ...
        XCTAssertFalse(try HookInstaller.plan(.claude, install: true, reporter: "/r/agentwatch-report").changed,
    ...
    func testClaudeInstallWithNoHooksKeyAndUninstallRemovesIt() throws {
        ...
        try HookInstaller.apply(try HookInstaller.plan(.claude, install: true, reporter: "/r/agentwatch-report"))
    ...
    func testCodexCreatesAndRemovesHooksJSON() throws {
        ...
        let plan = try HookInstaller.plan(.codex, install: true, reporter: "/r/agentwatch-report")
        let backups = try HookInstaller.apply(plan)
