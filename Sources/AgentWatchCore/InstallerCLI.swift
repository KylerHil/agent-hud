...
/// `agentwatch-report install-hooks | uninstall-hooks | hooks-status [--claude] [--codex] [--dry-run] [--yes]`
public enum InstallerCLI {
    ...
    static func change(_ targets: [HookInstaller.Target], install: Bool, dryRun: Bool, yes: Bool) -> Int32 {
        ...
        if install, plans.contains(where: { $0.target == .codex }) {
            ...
            run /hooks, and trust the AgentWatch entries. Until then AgentWatch falls back to Codex's session logs.
            """)
        }
    ...
    static func err(_ s: String) {
        FileHandle.standardError.write(Data("agentwatch-report: \(s)\n".utf8))
    }
}
