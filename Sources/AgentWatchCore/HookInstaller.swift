...
/// Merges AgentWatch hook entries into ~/.claude/settings.json and ~/.codex/hooks.json.
///
/// - Only entries whose command mentions `agentwatch-report` are ever added or removed.
/// - Install = strip ours, then append ours, so it is idempotent.
/// - Uses /usr/bin/jq so the user's key order and formatting survive.
/// - Every write is preceded by a timestamped backup in ~/.agentwatch/backups.
public enum HookInstaller {
    ...
    static let marker = "agentwatch-report"
    ...
    def isours: (type == "object") and ((.command? // "") | tostring | test("agentwatch-report"));
    ...
    public static func plan(_ target: Target, install: Bool, reporter: String = Paths.installedReporter.path) throws -> Plan {
        ...
        if target == .codex, let toml = try? String(contentsOf: Paths.codexConfig, encoding: .utf8) {
            ...
            if install, lines.contains(where: { $0.hasPrefix("[hooks") || $0.hasPrefix("[[hooks") }) {
                throw InstallError.blocked("""
                    ...
                    [hooks] in the same layer, so AgentWatch will not add hooks.json. Move those hooks into \
                    hooks.json first, or add AgentWatch's entries to config.toml by hand.
                    """)
            }
    ...
    public static func apply(_ plan: Plan, now: Date = Date()) throws -> [URL] {
        ...
        let tmp = dir.appendingPathComponent(".\(plan.file.lastPathComponent).agentwatch-tmp")
    ...
    /// Copies the reporter binary to its stable home (~/.agentwatch/bin) that hook commands point at.
    @discardableResult
    public static func installReporter(from source: URL) throws -> Bool {
        ...
        let tmp = Paths.binDir.appendingPathComponent(".agentwatch-report.tmp")
    ...
    static func unifiedDiff(_ a: String, _ b: String, path: String) -> String {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("agentwatch-diff-\(UUID().uuidString)")
