...
public enum HookPayload {
    public static func makeEvent(payload: [String: Any], agent: AgentKind, eventOverride: String?,
        ...
        if eventName == "UserPromptSubmit" {
            e.prompt = (str("user_input") ?? str("prompt")).map(stripContextTags)?.preview(120)
        }
    ...
    /// Editors prepend context blocks (`<ide_opened_file>…</ide_opened_file>`, `<ide_selection>…`) to prompts;
    /// the preview should show what the person typed.
