// agentwatch-report: called by Claude Code / Codex hooks.
//
// Reads the hook JSON from stdin, reduces it to an AgentEvent and appends one line to
// ~/.agentwatch/events.jsonl. It must never disturb the agent: it prints nothing to stdout,
// always exits 0, and kills itself after a hard deadline.
//
//   agentwatch-report --agent claude|codex [--event Name] [--no-pid]
//   agentwatch-report install-hooks|uninstall-hooks|hooks-status [...]   (see Installer)
